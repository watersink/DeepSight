...
class GuoanDetectorSkill(BaseSkill):
    ...
    def detect_frame(
        ...
        # 持续过暗时按冷却周期重复落库，避免画面一直 ALARM 却只写第一次
        has_dark_alarm = bool(
            state.alarm and (now - self._last_alert_ts >= self._repeat_sec)
        )
        if has_dark_alarm:
            self._last_alert_ts = now
        if state.status != prev_status or has_dark_alarm:
            self.log(
                "info",
                f"过暗检测: {state.status} emit={int(has_dark_alarm)} "
